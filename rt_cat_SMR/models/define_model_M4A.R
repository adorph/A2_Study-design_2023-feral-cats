...
NimModel <- nimbleCode({
  ...
  lam0.beta.sex ~ dnorm(0, 1) 
  sigma.beta0 ~ dnorm(5.719, 1 / ( 0.5 ^ 2) )# From data & literature, but more diffuse
  sigma.beta.sex ~ dnorm(0.651, 1 / ( 0.736 ^ 2) ) # From data & literature + 0.736 standard deviation
  ...
  thin.beta.hair ~ dnorm(0, 1) # Long_hair TRUE offset
  ...
  for(i in 1:M) {
    ...
    # Detection model:
    male.indicator[i] <- GetMale(G.true[i,1]) #convert male/female G.true to an indicator for males (female=0,male=1)
    for(j in 1:J){
      log(lam0[i,j]) <- lam0.beta0 + lam0.beta.sex*male.indicator[i]
    }
