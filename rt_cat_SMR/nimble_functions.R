# 
# This code was written for  and used in the journal article:
# 
# Cove MV, Herrmann V, Herrera DJ, Augustine BC, Flockhart DTT, 
# McShea WJ (2023) Counting the Capital's cats: Estimating drivers
# of abundance of free-roaming cats with a novel hierarchical approach.
# Ecological Applications, 33(2). doi: 10.1002/eap.2790
# 
# 
###################################################################
# Custom nimbleFunctions to fit categorical Random Thinning Model
###################################################################

#------------------------------------------------------------------
# Functions for masking activity centers
#------------------------------------------------------------------
...
GetDetectionRate <- nimbleFunction(
  run = function(s = double(1), lam0=double(1), sigma=double(0),
    ...
    if(z==1){
      ...
      ans[1:J] <- lam0[1:J]*exp(-d2[1:J]/(2*sigma^2))
      return(ans)
    }
  }
)
...
GetCoat <- nimbleFunction( #output indicator vector for 4 coat offsets
  run = function(x=double(0)){
    returnType(double(1))
    coat.indicator=numeric(4, value = 0)
    if(x==2){
      coat.indicator[1]=1
    }else if(x==3){
      coat.indicator[2]=1
...
#------------------------------------------------------------------
# Customer sampler to update latent IDs
#------------------------------------------------------------------
    ...
    for(l in 1:n.samples){#for all samples without known IDs
      ...
      if(denom>0){
        ...
        if(ID.cand[l]!=ID.curr[l]){
          ...
          ll.y.cand[swapped[1],this.j[l]]=dpois(y.true.cand[swapped[1],this.j[l]],model$lam[swapped[1],this.j[l]]*K1D[this.j[l]],log=TRUE)
          ll.y.cand[swapped[2],this.j[l]]=dpois(y.true.cand[swapped[2],this.j[l]],model$lam[swapped[2],this.j[l]]*K1D[this.j[l]],log=TRUE)
